...
class SubAgentManager {
  constructor({ prepareTask = null, createMcpConnection = () => null, onTaskAssigned = () => {}, onTaskEvent = () => {}, onDirectMessage = () => {}, onWorkerRegistered = () => {}, onWorkerDisconnected = () => {}, createArtifactUpload = () => null, discardArtifactUploads = async () => {}, materializeArtifacts = async (_task, work) => work } = {}) {
  ...
  queue({ agent, task, title, projectId, priority = "medium", timeoutMs = DEFAULT_TIMEOUT_MS }) {
    const messageId = `msg-${crypto.randomUUID()}`;
    const completion = this.delegate({ agent, task, title, projectId, priority, timeoutMs, messageId });
  ...
  async delegate({ agent, task, title, projectId, priority = "medium", timeoutMs = DEFAULT_TIMEOUT_MS, messageId: requestedMessageId } = {}) {
    const worker = this.workers.get(String(agent || ""));
    if (!worker) throw new Error(`Unknown or disconnected sub-agent: ${agent || "(empty)"}`);
