import { formatWorkerMessageContext } from './worker-message-context.js';
...
class SubAgentManager {
  constructor({ getMessageContext = () => null, prepareTask = null, createMcpConnection = () => null, onTaskAssigned = () => {}, onTaskEvent = () => {}, onDirectMessage = () => {}, onDirectMessageSent = () => {}, onWorkerRegistered = () => {}, onWorkerDisconnected = () => {}, createArtifactUpload = () => null, discardArtifactUploads = async () => {}, materializeArtifacts = async () => [] } = {}) {
    this.prepareTask = prepareTask;
    this.getMessageContext = getMessageContext;
    this.createMcpConnection = createMcpConnection;
    this.onTaskAssigned = onTaskAssigned;
    this.onTaskEvent = onTaskEvent;
    this.onDirectMessage = onDirectMessage;
    this.onDirectMessageSent = onDirectMessageSent;
    this.onWorkerRegistered = onWorkerRegistered;
    this.onWorkerDisconnected = onWorkerDisconnected;
    this.materializeArtifacts = materializeArtifacts;
    this.createArtifactUpload = createArtifactUpload;
    this.discardArtifactUploads = discardArtifactUploads;
  ...
  registerWorker(worker, transport) {
    ...
    const tokenName = String(worker?.tokenName || "").trim();
    ...
    this.workers.set(normalized.name, { ...normalized, ...(tokenName ? { tokenName } : {}), connectedAt: Date.now(), connectionId: transport.connectionId, send: transport.send, close: transport.close });
    const registered = this.publicWorker(this.workers.get(normalized.name));
    this.onWorkerRegistered(registered);
    return registered;
  }
  ...
  unregisterConnection(connectionId, reason = "Worker disconnected.") {
    for (const [name, worker] of this.workers) {
      if (worker.connectionId === connectionId) {
        ...
        this.onWorkerDisconnected({ ...this.publicWorker(worker), status: "offline", lastSeenAt: Date.now() });
      }
    }
  ...
  sendDirectMessage({ agent, text, timeoutMs = DEFAULT_DIRECT_MESSAGE_TIMEOUT_MS, purpose, officeTaskId, projectId } = {}) {
    ...
    const record = {
      ...
      ...(officeTaskId ? { officeTaskId: String(officeTaskId) } : {}),
    };
    ...
    try {
      const context = this.getMessageContext(record);
      const text = context ? formatWorkerMessageContext(context, normalizedText) : normalizedText;
      worker.send({
        ...(context ? { context, projectId: context.project.id, officeTaskId: context.task?.id || null } : {}),
        type: "direct_message",
        ...(projectId ? { projectId } : {}),
        message: { messageId, role: "user", parts: [{ kind: "text", mimeType: "text/plain", text }] },
      });
    } catch (error) {
    ...
    try { this.onDirectMessageSent(receipt); } catch { /* Observers cannot break delivery. */ }
    return receipt;
  }
  ...
  queue({ agent, task, title, projectId, officeTaskId, priority = "medium", timeoutMs = DEFAULT_TIMEOUT_MS }) {
    const messageId = `msg-${crypto.randomUUID()}`;
    const completion = this.delegate({ agent, task, title, projectId, officeTaskId, priority, timeoutMs, messageId });
  ...
  async delegate({ agent, task, title, projectId, officeTaskId, priority = "medium", timeoutMs = DEFAULT_TIMEOUT_MS, messageId: requestedMessageId } = {}) {
    const worker = this.workers.get(String(agent || ""));
    if (!worker) throw new Error(`Unknown or disconnected sub-agent: ${agent || "(empty)"}`);
    ...
    const record = {
      ...
      state: "assigned", taskId, ...(officeTaskId ? { officeTaskId } : {}), title: String(title || "").trim() || task.trim(), workerUrl: worker.url,
    ...
    try {
      const text = this.prepareTask ? await this.prepareTask({ ...record, task: task.trim() }) : task.trim();
      if (TERMINAL_TASK_STATES.has(record.state)) return result;
      if (typeof text !== "string" || !text.trim() || text.length > MAX_TASK_LENGTH) throw new Error("Prepared worker task must contain between 1 and 100000 characters.");
      const { resolve: _resolve, timer: _timer, ...uploadTask } = record;
      const artifactUpload = this.createArtifactUpload({ ...uploadTask });
      const mcpServers = this.createMcpConnection(uploadTask, artifactUpload);
      const context = this.getMessageContext(record);
      worker.send({
        ...
        message: { messageId, role: "manager", parts: [{ kind: "text", mimeType: "text/plain", text }] },
      });
  ...
  cancelTask({ messageId, taskId, reason = "Task stopped by the office manager." } = {}) {
    ...
    if (TERMINAL_TASK_STATES.has(record.state)) {
      const { resolve, timer, ...task } = record;
    ...
    const { resolve, timer, ...task } = record;
    return { ...task, ...(cancelDeliveryError ? { cancelDeliveryError } : {}) };
  }
  ...
  receiveUpdate(agent, payload, connectionId) {
    ...
    if (state === "completed" && payload.artifacts !== undefined
      && (!Array.isArray(payload.artifacts) || payload.artifacts.length)) {
      throw new Error("Worker-hosted artifacts are no longer supported. POST raw file bytes to artifactUpload.url, then complete with uploadedArtifactIds. The task remains active so you can upload and retry.");
    }
    const uploadedArtifactIds = state === "completed" && payload.uploadedArtifactIds !== undefined
      ? payload.uploadedArtifactIds
      : [];
    if (state === "failed") {
    ...
    return Promise.resolve().then(() => this.materializeArtifacts({ ...record }, uploadedArtifactIds)).then((storedWork) => {
      this.finish(record, { ok: true, status: state, text: messageText(payload), deliveredWork: storedWork });
      return { taskId: record.taskId, state: record.state };
    }).catch((error) => {
  ...
  finish(record, output) {
    ...
    Promise.resolve(this.discardArtifactUploads(record.taskId)).catch(() => {});
