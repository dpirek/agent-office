...
class SubAgentManager {
  constructor({ onTaskAssigned = () => {}, onTaskEvent = () => {} } = {}) {
  ...
  unregisterConnection(connectionId, reason = "Worker disconnected.") {
    ...
    for (const record of this.tasks.values()) {
      if (record.connectionId === connectionId && !["completed", "failed", "timed_out"].includes(record.state)) {
  ...
  async delegate({ agent, task, priority = "medium", timeoutMs = DEFAULT_TIMEOUT_MS, messageId: requestedMessageId } = {}) {
    ...
    const record = {
      ...
      state: "assigned", taskId, title: task.trim(), workerUrl: worker.url,
    ...
    try {
      worker.send({
        ...
        message: { messageId, role: "manager", parts: [{ kind: "text", mimeType: "text/plain", text: record.title }] },
      });
  ...
  receiveUpdate(agent, payload, connectionId) {
    ...
    const output = state === "completed"
      ? { ok: true, status: state, text: messageText(payload), artifacts, deliveredWork: work }
      : { ok: false, status: state, error: payload.error?.message || payload.error || messageText(payload) || "Sub-agent task failed." };
    this.finish(record, output);
    return { taskId: record.taskId, state: record.state };
  }

  finish(record, output) {
    if (["completed", "failed", "timed_out"].includes(record.state)) return;
...
export { DEFAULT_TIMEOUT_MS, MAX_TIMEOUT_MS, SubAgentManager, normalizeSubAgentWorkers };
