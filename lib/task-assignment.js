const ACTIVE_TASK_STATES = new Set(["assigned", "working", "running", "pending"]);

export function selectAutoAssignedWorker(workers, tasks) {
  if (!workers.length) throw new Error("Connect at least one WebSocket worker before assigning a task.");
