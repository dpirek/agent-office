...
export function createUserStore(databasePath, { now = Date.now, onRevoke = () => {} } = {}) {
  ...
  const needsMembershipMigration = !db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='project_memberships'").get();
  db.exec(`CREATE TABLE IF NOT EXISTS project_memberships (
    user_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    project_id TEXT NOT NULL, PRIMARY KEY(user_id, project_id)
  )`);
  if (needsMembershipMigration) db.exec(`INSERT OR IGNORE INTO project_memberships(user_id,project_id)
    SELECT u.id, i.project_id FROM project_invitations i JOIN users u ON u.email=i.email
    WHERE i.used_at IS NOT NULL AND u.project_ids IS NULL`);
  function recordMembership(userId, projectId) {
    db.prepare('INSERT OR IGNORE INTO project_memberships(user_id,project_id) VALUES(?,?)').run(userId, projectId);
  }
  ...
  function consumeInvitation(code, user) {
    ...
    recordMembership(user.id, invite.project_id);
  ...
  return {
    ...
    projectMembers(project) {
      const explicit = new Set(db.prepare('SELECT user_id FROM project_memberships WHERE project_id=?').all(project.id).map(row => row.user_id));
      return this.list().filter(user => !user.disabled && ['admin', 'member'].includes(user.role)
        && (user.id === project.ownerId || user.projectIds?.includes(project.id)
          || (user.projectIds == null && explicit.has(user.id))));
    },
    getInvitation(code) {
    ...
    update(id, { role, disabled, projectIds }) {
      const result = transaction(() => {
        ...
        if (Array.isArray(projectIds)) {
          db.prepare('DELETE FROM project_memberships WHERE user_id=?').run(id);
          for (const projectId of new Set(projectIds)) recordMembership(id, projectId);
        }
