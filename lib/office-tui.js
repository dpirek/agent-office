...
import { renderOfficeArt } from './office-tui-art.js';

const VIEWS = ['Overview', 'Workers', 'Tasks', 'Logs', 'Office'];
...
export function renderOfficeTui(snapshot, { columns = 100, rows = 30, view = 0, offset = 0, paused = false, now = Date.now() } = {}) {
  ...
  if (view === 4) {
    content.push(`OFFICE FLOOR  |  Manager ${snapshot.managerBusy ? 'working' : 'idle'}  |  ${workers.filter(worker => worker.status === 'connected').length} workers online`);
    if (width >= 38 && height >= 20) {
      const scene = renderOfficeArt(snapshot, Math.min(width, 110), Math.min(25, height - 8));
      content.push(...scene.lines, `AMBER busy | MINT online | GRAY offline / empty | ${scene.visible}/${scene.total} desks occupied`, 'Manager sits first. Press 2 for the full worker roster.');
    } else content.push('Enlarge terminal to 39 x 20 for pixel art.', ...workerLines);
  }
  else if (view === 1) content.push('STATUS   WORKER                    MODEL', ...workerLines.length ? workerLines : ['No workers registered.']);
  ...
  lines.push('-'.repeat(width), `1-5 views | Up/Down PgUp/PgDn scroll | Space pause | q hide | Ctrl+C stop  ${scroll + 1}/${Math.max(1, maxOffset + 1)}`);
  return { lines: lines.slice(0, height).map(line => typeof line === 'object'
    ? line.art + ' '.repeat(Math.max(0, width - stripVTControlCharacters(line.art).length))
    : fit(line, width)), maxOffset };
}
...
export function startOfficeTui({ getSnapshot, input = process.stdin, output = process.stdout, intervalMs = 1000, consoleTarget = console, signals = process, interrupt = () => process.kill(process.pid, 'SIGINT') }) {
  ...
  let active = true, view = 4, offset = 0, paused = false, snapshot, maxOffset = 0;
  ...
  function keypress(text, key = {}) {
    ...
    if (/^[1-5]$/.test(text || '')) { view = Number(text) - 1; offset = 0; }
