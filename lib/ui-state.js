...
export function createUiStateStore(databasePath) {
  ...
  const insertOfficeTask = database.prepare(`
    INSERT INTO office_tasks
      (id, title, priority, status, depends_on, created_at, updated_at, artifacts, project_id)
    VALUES (?, ?, ?, 'pending', ?, ?, ?, '[]', ?)
  `);
  ...
  return {
    ...
    createOfficeTask({ title, priority = "medium", dependsOn = [], projectId = "central-office" }) {
      this.requireProject(projectId);
      const id = crypto.randomUUID();
      const now = Date.now();
      insertOfficeTask.run(id, String(title), priority, JSON.stringify(dependsOn), now, now, projectId);
