...
export function createUiStateStore(databasePath) {
  ...
  const insertOfficeTask = database.prepare(`
    INSERT INTO office_tasks
      (id, title, priority, status, created_at, updated_at, artifacts)
    VALUES (?, ?, ?, 'pending', ?, ?, '[]')
  `);
  ...
  const upsertWorkerToken = database.prepare(`
    INSERT INTO worker_credentials (id, token, updated_at) VALUES (1, ?, ?)
  ...
  return {
    ...
    createOfficeTask({ title, priority = "medium" }) {
      const id = crypto.randomUUID();
      const now = Date.now();
      insertOfficeTask.run(id, String(title), priority, now, now);
    ...
    setWorkerToken(token) {
      ...
      upsertWorkerToken.run(normalized, Date.now());
      return true;
    },
