...
export function createUiStateStore(databasePath, { liveLoggingEnabled = true } = {}) {
  ...
  database.exec(`
    ...
    CREATE TABLE IF NOT EXISTS observability_providers (
      id TEXT PRIMARY KEY,
  ...
  const workerRegistryColumns = database.prepare("SELECT name FROM pragma_table_info('worker_registry')").all();
  if (!workerRegistryColumns.some(({ name }) => name === "connectivity_announced_at")) {
    database.exec("ALTER TABLE worker_registry ADD COLUMN connectivity_announced_at INTEGER");
  }
  ...
  database.exec(`
    ...
    INSERT OR IGNORE INTO projects (id, name, created_at, updated_at)
      VALUES ('central-office', 'Central Office', 0, 0);
  `);
  if (!database.prepare("PRAGMA table_info(office_chat_messages)").all().some(column => column.name === 'reply_to_id')) {
    database.exec("ALTER TABLE office_chat_messages ADD COLUMN reply_to_id TEXT");
  }
  database.exec("CREATE INDEX IF NOT EXISTS office_chat_reply ON office_chat_messages(reply_to_id)");
  const projectColumns = database.prepare("PRAGMA table_info(projects)").all();
  if (!projectColumns.some(column => column.name === 'is_public')) database.exec("ALTER TABLE projects ADD COLUMN is_public INTEGER NOT NULL DEFAULT 0");
  if (!projectColumns.some(column => column.name === 'owner_id')) database.exec("ALTER TABLE projects ADD COLUMN owner_id TEXT");
  if (!database.prepare("SELECT name FROM pragma_table_info('office_tasks')").all().some((column) => column.name === "description")) {
    database.exec("ALTER TABLE office_tasks ADD COLUMN description TEXT NOT NULL DEFAULT ''");
  }
  for (const table of ["office_tasks", "office_chat_messages", "periodic_operations", "office_memory"]) {
    const columns = database.prepare(`SELECT name FROM pragma_table_info('${table}')`).all();
    if (!columns.some((column) => column.name === "project_id")) {
      database.exec(`ALTER TABLE ${table} ADD COLUMN project_id TEXT NOT NULL DEFAULT 'central-office'`);
      if (table === "office_memory") database.exec(`
        UPDATE office_memory SET project_id = COALESCE(
          (SELECT project_id FROM office_tasks WHERE office_tasks.id = office_memory.source_id
            OR office_tasks.message_id = office_memory.source_id OR office_tasks.worker_task_id = office_memory.source_id
            OR office_tasks.message_id = json_extract(office_memory.details, '$.messageId') LIMIT 1),
          'central-office');
      `);
    }
    database.exec(`CREATE INDEX IF NOT EXISTS ${table}_project ON ${table}(project_id, created_at)`);
    database.exec(`
      CREATE TRIGGER IF NOT EXISTS ${table}_project_insert BEFORE INSERT ON ${table}
      WHEN NOT EXISTS (SELECT 1 FROM projects WHERE id = NEW.project_id)
      BEGIN SELECT RAISE(ABORT, 'Unknown project'); END;
      CREATE TRIGGER IF NOT EXISTS ${table}_project_update BEFORE UPDATE OF project_id ON ${table}
      WHEN NOT EXISTS (SELECT 1 FROM projects WHERE id = NEW.project_id)
      BEGIN SELECT RAISE(ABORT, 'Unknown project'); END;
    `);
  }
  ...
  const insertPeriodicOperation = database.prepare(`
    INSERT INTO periodic_operations
      (id, name, task, agent, interval_value, interval_unit, priority, enabled,
       created_at, updated_at, last_run_at, next_run_at, last_status, last_error, project_id)
  ...
  const insertOfficeMemory = database.prepare(`
    INSERT INTO office_memory
      (id, kind, status, title, summary, agent, source_id, details, artifacts, occurred_at, created_at, project_id)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  const insertOfficeTask = database.prepare(`
    INSERT INTO office_tasks
      (id, title, priority, status, depends_on, created_at, updated_at, artifacts, project_id, description)
    VALUES (?, ?, ?, 'pending', ?, ?, ?, '[]', ?, ?)
  `);
  ...
  const insertOfficeChatMessage = database.prepare(`
    INSERT INTO office_chat_messages
      (id, author, username, kind, text, artifacts, task_id, created_at, project_id, reply_to_id)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  const readOfficeChatMessages = ({ after = 0, limit = 200, projectId = "central-office" } = {}) => {
    const boundedLimit = Math.max(1, Math.min(500, Number(limit) || 200));
    return database.prepare(`
      SELECT * FROM office_chat_messages
      WHERE created_at > ? AND project_id = ?
      ORDER BY created_at ${after > 0 ? "ASC" : "DESC"}, id ${after > 0 ? "ASC" : "DESC"}
      LIMIT ?
    `).all(Math.max(0, Number(after) || 0), projectId, boundedLimit).sort((a, b) => a.created_at - b.created_at || a.id.localeCompare(b.id)).map((row) => ({
      id: row.id,
      author: row.author,
  ...
  const readOfficeTasks = ({ limit = 200, status, projectId, id } = {}) => {
    const boundedLimit = Math.max(1, Math.min(500, Number(limit) || 200));
    const conditions = [], values = [];
    if (id) { conditions.push("id = ?"); values.push(String(id)); }
    if (status) { conditions.push("status = ?"); values.push(String(status)); }
    if (projectId !== undefined) { conditions.push("project_id = ?"); values.push(String(projectId)); }
    const rows = database.prepare(`SELECT * FROM office_tasks ${conditions.length ? `WHERE ${conditions.join(" AND ")}` : ""} ORDER BY created_at DESC LIMIT ?`).all(...values, boundedLimit);
    return rows.map((row) => ({
      id: row.id,
      title: row.title,
  ...
  const readOfficeMemory = ({ limit = 100, kind, status, query, projectId } = {}) => {
    const conditions = [];
    const values = [];
    ...
    const rows = database.prepare(`
      SELECT id, kind, status, title, summary, agent, source_id, details, artifacts,
        occurred_at, created_at, project_id FROM office_memory
  ...
  const readPeriodicOperations = ({ projectId } = {}) => database.prepare(`
    SELECT id, name, task, agent, interval_value, interval_unit, priority, enabled,
      created_at, updated_at, last_run_at, next_run_at, last_status, last_error, project_id
    FROM periodic_operations ${projectId ? "WHERE project_id = ?" : ""} ORDER BY enabled DESC, next_run_at, lower(name)
  ...
  return {
    ...
    factoryReset() {
      ...
      try {
        database.exec(`
          ...
          DELETE FROM observability_providers;
          DELETE FROM office_memory;
          DELETE FROM office_tasks;
          DELETE FROM projects WHERE id != 'central-office';
    ...
    getPeriodicOperations(filters = {}) {
      return readPeriodicOperations(filters);
    },
    createPeriodicOperation(operation) {
      const projectId = operation.projectId || "central-office";
      ...
      insertPeriodicOperation.run(
        ...
        now, now, operation.nextRunAt, projectId,
      );
    ...
    recordOfficeMemory(entry) {
      const projectId = entry.projectId || "central-office";
      this.requireProject(projectId);
      const now = Date.now();
    ...
    getProjects() {
      return database.prepare("SELECT id, name, description, status, is_public AS isPublic, owner_id AS ownerId, created_at AS createdAt, updated_at AS updatedAt FROM projects ORDER BY created_at, id").all().map(project => ({ ...project, isPublic: Boolean(project.isPublic) }));
    },
    requireProject(id = "central-office") {
      const project = this.getProjects().find((entry) => entry.id === id);
      if (!project) throw new Error(`Unknown project: ${id}`);
      return project;
    },
    createProject({ name, description = "", isPublic = false, ownerId = null } = {}) {
      const normalized = String(name || "").trim();
      if (!normalized || normalized.length > 100) throw new Error("Project name must be between 1 and 100 characters.");
      if (String(description).length > 10000) throw new Error("Project description is too long.");
      const id = crypto.randomUUID(), now = Date.now();
      database.prepare("INSERT INTO projects (id, name, description, is_public, owner_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)").run(id, normalized, String(description), Number(isPublic), ownerId, now, now);
      return this.requireProject(id);
    },
    updateProject({ id, name, description, status } = {}) {
      const current = this.requireProject(id);
      const next = { ...current, ...(name !== undefined ? { name: String(name).trim() } : {}), ...(description !== undefined ? { description: String(description) } : {}), ...(status !== undefined ? { status } : {}) };
      if (!next.name || next.name.length > 100 || next.description.length > 10000) throw new Error("Invalid project name or description.");
      if (!["active", "paused", "completed", "archived"].includes(next.status)) throw new Error("Invalid project status.");
      database.prepare("UPDATE projects SET name = ?, description = ?, status = ?, updated_at = ? WHERE id = ?").run(next.name, next.description, next.status, Date.now(), id);
      return this.requireProject(id);
    },
    getOfficeTasks(filters = {}) {
      return readOfficeTasks(filters);
    },
    createOfficeTask({ title, description = "", priority = "medium", dependsOn = [], projectId = "central-office" }) {
      this.requireProject(projectId);
      const id = crypto.randomUUID();
      const now = Date.now();
      insertOfficeTask.run(id, String(title), priority, JSON.stringify(dependsOn), now, now, projectId, String(description));
      return readOfficeTasks({ id })[0];
    },
    assignOfficeTask(id, { agent, messageId }) {
      ...
      return readOfficeTasks({ id: String(id) })[0];
    },
    ...
    getOfficeChatMessage(id, projectId) {
      const row = database.prepare('SELECT * FROM office_chat_messages WHERE id=? AND project_id=?').get(id, projectId);
      return row ? { id: row.id, author: row.author, username: row.username, kind: row.kind, text: row.text, artifacts: JSON.parse(row.artifacts), taskId: row.task_id, replyToId: row.reply_to_id || null, projectId: row.project_id, createdAt: row.created_at } : null;
    },
    getOfficeChatThread(id, projectId) {
      let root = this.getOfficeChatMessage(id, projectId);
      if (!root) throw new Error('Reply target not found in this project.');
      while (root.replyToId) root = this.getOfficeChatMessage(root.replyToId, projectId);
      const ids = database.prepare(`WITH RECURSIVE thread(id) AS (
        SELECT id FROM office_chat_messages WHERE id=? AND project_id=?
        UNION ALL SELECT m.id FROM office_chat_messages m JOIN thread t ON m.reply_to_id=t.id WHERE m.project_id=?
      ) SELECT m.id FROM office_chat_messages m JOIN thread t ON m.id=t.id ORDER BY m.created_at DESC, m.id DESC LIMIT 100`).all(root.id, projectId, projectId);
      const messages = ids.reverse().map(row => this.getOfficeChatMessage(row.id, projectId));
      if (!messages.some(message => message.id === root.id)) messages.unshift(root);
      return messages;
    },
    ...
    recordSystemActivity(entry) {
      if (!liveLoggingEnabled) return null;
      entry = sanitizeActivity(entry);
      const id = String(entry?.id || crypto.randomUUID());
    ...
    getObservabilityProviders() {
      return database.prepare('SELECT * FROM observability_providers ORDER BY rowid').all().map(row => ({ ...JSON.parse(row.configuration), id: row.id, cursor: row.cursor, status: JSON.parse(row.status) }));
    },
    addObservabilityProvider(configuration) {
      const id = crypto.randomUUID();
      const cursor = database.prepare('SELECT COALESCE(MAX(rowid), 0) AS cursor FROM system_activity_logs').get().cursor;
      database.prepare('INSERT INTO observability_providers (id, configuration, cursor) VALUES (?, ?, ?)').run(id, JSON.stringify(configuration), cursor);
      return this.getObservabilityProviders().find(provider => provider.id === id);
    },
    setObservabilityEnabled(id, enabled) {
      const provider = this.getObservabilityProviders().find(provider => provider.id === id);
      if (!provider) throw new Error('Provider not found.');
      const { cursor, status, id: _id, ...configuration } = provider;
      database.prepare('UPDATE observability_providers SET configuration = ? WHERE id = ?').run(JSON.stringify({ ...configuration, enabled }), id);
    },
    deleteObservabilityProvider(id) {
      database.prepare('DELETE FROM observability_providers WHERE id = ?').run(id);
    },
    updateObservabilityDelivery(id, update) {
      const provider = this.getObservabilityProviders().find(provider => provider.id === id);
      if (!provider) return;
      const { cursor = provider.cursor, ...status } = update;
      database.prepare('UPDATE observability_providers SET cursor = ?, status = ? WHERE id = ?').run(cursor, JSON.stringify({ ...provider.status, ...status }), id);
    },
    getObservabilityBatch(cursor, limit = 100) {
      return database.prepare('SELECT rowid AS sequence, * FROM system_activity_logs WHERE rowid > ? ORDER BY rowid LIMIT ?').all(cursor, limit).map(row => ({ sequence: row.sequence, id: row.id, category: row.category, source: row.source, message: row.message, tone: row.tone, metadata: JSON.parse(row.metadata), createdAt: row.created_at }));
    },
    getWorkerToken() {
    ...
    announceWorkerConnectivity(name, postMessage) {
      database.exec("BEGIN IMMEDIATE");
      try {
        const result = database.prepare(`UPDATE worker_registry SET connectivity_announced_at = ?
          WHERE name = ? AND connectivity_announced_at IS NULL`).run(Date.now(), name);
        if (result.changes) postMessage();
        database.exec("COMMIT");
    ...
    createOfficeChatMessage(message) {
      this.requireProject(message.projectId || "central-office");
      if (message.replyToId && (typeof message.replyToId !== 'string' || !this.getOfficeChatMessage(message.replyToId, message.projectId || 'central-office'))) throw new Error('Reply target not found in this project.');
      ...
      insertOfficeChatMessage.run(
        ...
        createdAt,
        message.projectId || "central-office",
        message.replyToId || null,
      );
      this.recordSystemActivity({ category: 'chat', source: message.author || 'Chat', message: `Chat message · ${String(message.text).slice(0, 240)}`, metadata: { projectId: message.projectId || 'central-office', messageId: id, taskId: message.taskId || null, replyToId: message.replyToId || null, kind: message.kind, text: message.text, artifacts: message.artifacts || [] } });
      return readOfficeChatMessages({ after: Math.max(0, createdAt - 1), limit: 500, projectId: message.projectId || "central-office" })
