...
function parseStoredMcpConfig(raw, source) {
  const content = String(raw || '').replace(/^\uFEFF/, '').trim();
  // JSON arrays contain server objects; TOML starts with named table headers.
  if (content.startsWith('{') || /^\[\s*(?:\{|\])/.test(content)) return parseEnvConfig(content, source);
  return parseTomlConfig(content, source);
}
...
class McpHttpClient {
  ...
  async connect() {
    this.onInfo(`Connecting HTTP MCP server ${this.server.server_label}.`);
    try {
      this.mode = "modern";
      this.protocolVersion = MCP_STATELESS_PROTOCOL_VERSION;
    ...
    this.onInfo(`Connected HTTP MCP server ${this.server.server_label}.`);
  }

  async listTools() {
    this.onInfo(`Loading tools from MCP server ${this.server.server_label}.`);
    let result = this.cachedTools || await this.request("tools/list", {});
    this.cachedTools = null;
    const tools = [];
    ...
    for (;;) {
      ...
      if (tools.length > 1000) throw new Error("Too many MCP tools.");
      if (!result.nextCursor) return tools;
      if (cursors.has(result.nextCursor) || cursors.size >= 10) throw new Error("Invalid MCP tools pagination.");
      cursors.add(result.nextCursor);
      result = await this.request("tools/list", { cursor: result.nextCursor });
    }
  }
...
async function loadHttpServerTools({ server, approve, onInfo, fetchImpl }) {
  const client = new McpHttpClient({ server, onInfo, fetchImpl });
...
async function loadMcpTools({
  ...
  if (configContent !== undefined) {
    configs.push({
      ...
      config: parseStoredMcpConfig(configContent, "SQLite MCP configuration"),
    });
  } else if (root) {
  ...
  for (const { source, config } of configs) {
    ...
    for (const [index, server] of remoteMcpServers(config, source).entries()) {
      ...
      if (tool.server_url && (discoverRemoteTools || isLoopbackUrl(tool.server_url))) {
        const discovered = await loadHttpServerTools({ server: tool, approve, onInfo, fetchImpl });
        onInfo(`Loaded ${discovered.length} callable tools from MCP server ${tool.server_label}.`);
        tools.push(...discovered);
      } else {
        if (!supportsNativeMcp) {
          throw new Error(`MCP server ${tool.server_label} requires native MCP support. Configure an HTTP server URL for this model provider.`);
        }
...
// Discovery only: never execute a remote tool or save the supplied configuration.
export async function testMcpServer(server, { fetchImpl = fetch } = {}) {
  const client = new McpHttpClient({ server, fetchImpl: (url, options) => fetchImpl(url, { ...options, redirect: 'error' }) });
  try {
    await client.connect();
    const tools = [];
    let result = client.cachedTools || await client.request('tools/list', {}, 10_000);
    const cursors = new Set();
    for (;;) {
      if (!Array.isArray(result.tools)) throw new Error('Invalid tools list.');
      tools.push(...result.tools.map(({ name, description, inputSchema }) => ({ name, description, inputSchema })));
      if (tools.length > 1000) throw new Error('Too many tools.');
      if (!result.nextCursor) break;
      if (cursors.has(result.nextCursor) || cursors.size >= 10) throw new Error('Invalid tools pagination.');
      cursors.add(result.nextCursor);
      result = await client.request('tools/list', { cursor: result.nextCursor }, 10_000);
    }
    const secrets = Object.values(server.headers || {}).filter(Boolean).flatMap(value => /^Bearer /i.test(value) ? [value, value.slice(7)] : [value]).filter(Boolean);
    const redact = value => {
    ...
    // Upstream errors may echo authentication headers or response bodies.
    throw new Error(error.status ? `MCP connection failed (HTTP ${error.status}). Check the URL and headers.` : 'Could not discover MCP methods. Check the URL, headers, and whether the server is reachable.');
  } finally {
    if (client.sessionId) {
      try { const response = await fetchImpl(server.server_url, { method: 'DELETE', headers: client.baseHeaders('', {}), redirect: 'error', signal: AbortSignal.timeout(3000) }); await response.body?.cancel(); } catch { /* Session cleanup is best-effort. */ }
    }
  }
}
