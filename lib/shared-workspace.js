...
function createSharedWorkspace({ root } = {}) {
  const workspaceRoot = path.resolve(root);
  let deliveryQueue = Promise.resolve();

  async function storeDelivery(task, uploadedWork = []) {
    if (!uploadedWork.length) return [];
  ...
  async function storeArtifact(artifact, content, staging, folder, stored) {
    ...
    if (isZip) {
      const archive = path.join(staging, `.${filename}.upload`);
  ...
  function storeTaskArtifacts(task, uploadedWork = []) {
    const delivery = deliveryQueue.then(() => storeDelivery(task, uploadedWork));
...
export { createSharedWorkspace, extractZip, MAX_EXTRACTED_BYTES };
