...
function createSharedWorkspace({ root, fetchImpl = fetch } = {}) {
  ...
  async function storeTaskArtifacts(task, deliveredWork = []) {
    if (!deliveredWork.length) return [];
    await fs.mkdir(workspaceRoot, { recursive: true });
    const folder = taskFolderName(task);
    const destination = path.join(workspaceRoot, folder);
    const staging = path.join(workspaceRoot, `.${folder}.${crypto.randomUUID()}.incoming`);
