...
function taskFolderName(task) {
  const title = safeSegment(task.title, "task");
  const id = safeSegment(task.taskId || task.messageId || crypto.randomUUID(), "delivery");
  return `${title}--${id}`;
}
...
function localArtifact(folder, relativePath, source) {
  ...
  return {
    ...
    uri: `/api/shared-workspace-file?path=${encodeURIComponent(storedPath)}`,
...
function createSharedWorkspace({ root, fetchImpl = fetch } = {}) {
  ...
  async function storeTaskArtifacts(task, deliveredWork = [], uploadedWork = []) {
    if (!deliveredWork.length && !uploadedWork.length) return [];
    await fs.mkdir(workspaceRoot, { recursive: true });
    if (task.projectId) await projectWorkspace(workspaceRoot, task.projectId);
    const folder = task.projectId ? path.posix.join(task.projectId, taskFolderName(task)) : taskFolderName(task);
    const destination = path.join(workspaceRoot, folder);
    const staging = path.join(workspaceRoot, `.${taskFolderName(task)}.${crypto.randomUUID()}.incoming`);
...
export { createSharedWorkspace, extractZip, MAX_DOWNLOAD_BYTES, MAX_EXTRACTED_BYTES, taskFolderName };
