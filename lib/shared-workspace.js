...
async function download(uri, fetchImpl, { workerUrl, getWorkerToken }) {
  let url = new URL(uri);
  ...
  for (let redirects = 0; ; redirects++) {
    ...
    if (response.status === 401 && url.origin === trustedOrigin) {
...
function createSharedWorkspace({ root, fetchImpl = fetch, getWorkerToken = () => '' } = {}) {
  ...
  async function storeDelivery(task, deliveredWork = [], uploadedWork = []) {
    ...
    try {
      for (const artifact of deliveredWork) {
        const content = await download(artifact.uri, fetchImpl, { workerUrl: task.workerUrl, getWorkerToken });
