...
function workerHttpOrigin(workerUrl) {
  try {
...
async function download(uri, fetchImpl, { workerUrl, getWorkerToken, getOfficeOrigins, projectId }) {
  let url = new URL(uri);
  const trustedOrigin = workerHttpOrigin(workerUrl);
  const signal = AbortSignal.timeout(120_000);
  let response;
  for (let redirects = 0; ; redirects++) {
    if (!['http:', 'https:'].includes(url.protocol) || url.username || url.password) throw new Error('Artifact download requires an HTTP(S) URL without embedded credentials.');
    const officeAsset = url.pathname === '/api/workspace-file-asset' && getOfficeOrigins().includes(url.origin);
    if (officeAsset && url.searchParams.has('projectId') && url.searchParams.get('projectId') !== projectId) throw new Error('Artifact belongs to a different project.');
    const officeToken = officeAsset ? getWorkerToken() : '';
    response = await fetchImpl(url.href, { signal, redirect: 'manual', ...(officeToken ? { headers: { Authorization: `Bearer ${officeToken}` } } : {}) });
    // Only the registered worker's server may receive its shared office key.
    // Public/signed URLs remain anonymous; redirects are checked individually.
    if (response.status === 401 && !officeToken && url.origin === trustedOrigin) {
      const token = getWorkerToken();
      if (token) {
        await response.body?.cancel();
        response = await fetchImpl(url.href, { signal, redirect: 'manual', headers: { Authorization: `Bearer ${token}` } });
      }
    }
    if (![301, 302, 303, 307, 308].includes(response.status)) break;
    const location = response.headers.get('location');
    await response.body?.cancel();
    if (!location || redirects >= 5) throw new Error('Artifact download redirect is missing or exceeds the limit.');
    url = new URL(location, url);
  }
  if (!response.ok) {
    await response.body?.cancel();
    const hint = [401, 403].includes(response.status)
      ? ' The file server rejected authentication. Upload raw bytes to the assignment artifactUpload URL and return uploadedArtifactIds, or serve the file from the registered worker origin using the current worker key.' : '';
    throw new Error(`Artifact download failed (${response.status}).${hint}`);
  }
...
function createSharedWorkspace({ root, fetchImpl = fetch, getWorkerToken = () => '', getOfficeOrigins = () => [] } = {}) {
  ...
  async function storeDelivery(task, deliveredWork = [], uploadedWork = []) {
    ...
    try {
      for (const artifact of deliveredWork) {
        const content = await download(artifact.uri, fetchImpl, { workerUrl: task.workerUrl, getWorkerToken, getOfficeOrigins, projectId: folder });
