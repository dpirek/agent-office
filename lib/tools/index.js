...
import { createOfficeMemoryTool } from "./office-memory.js";
import { createOfficeTasksTool } from "./office-tasks.js";
...
const builtInToolFactories = Object.freeze([
  ["read_office_memory", createOfficeMemoryTool],
  ["manage_office_tasks", createOfficeTasksTool],
...
function createToolRegistry(initialFactories = []) {
  ...
  function createTools({ root, approve = async () => false, subAgentManager, uiStateStore } = {}) {
    const context = { ...createWorkspaceContext({ root, approve }), subAgentManager, uiStateStore };
