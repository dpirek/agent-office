...
function createOfficeTasksTool({ uiStateStore, subAgentManager }) {
  ...
  return {
    ...
    parameters: objectSchema({
      ...
      title: { type: ["string", "null"], description: "Concise task title required for create." },
      description: { type: ["string", "null"], description: "Detailed instructions for create: objective, relevant context, constraints, deliverables, acceptance criteria, and exact input/output paths in app/, docs/, designs/, research/, or scripts/. Reuse project files; never request a per-task delivery folder. Provide enough detail for the worker to act without the conversation." },
    ...
    async execute({ action, task_id: taskId, title, description, priority, depends_on: dependsOn, agent, status, reason }) {
      ...
      try {
        ...
        if (action === "create") return { ok: true, task: createOfficeTask(uiStateStore, { title, description, priority: priority || "medium", dependsOn: dependsOn || [] }) };
        if (action === "assign") {
