...
export function resolvePublicPath(publicDir, urlPath) {
  ...
  const target = decoded === "/" ? "/landing.html" : decoded;
...
export async function serveStatic(req, res, publicDir) {
  ...
  if (requestUrl.pathname === "/favicon.ico") {
    redirect(res, "/assets/logo.svg");
    return;
  }

  const authPage = requestUrl.pathname.match(/^\/(login|register)\/?$/);
  const filePath = resolvePublicPath(publicDir, authPage ? `/${authPage[1]}.html` : requestUrl.pathname);
  if (!filePath) {
  ...
  try {
    ...
    if (error.code === "ENOENT" || error.code === "ENOTDIR") {
      if (req.method === "GET" && !path.extname(requestUrl.pathname)) {
        try {
          const indexPath = path.join(path.resolve(publicDir), "index.html");
          respond(res, 200, await fs.readFile(indexPath), {
