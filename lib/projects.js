...
export function projectStore(store, projectId) {
  ...
  return new Proxy(store, {
    get(target, key) {
      ...
      if (key === "getOfficeMemory" || key === "getPeriodicOperations") return (filters = {}) => target[key]({ ...filters, projectId });
      if (key === "recordOfficeMemory" || key === "createPeriodicOperation") return (value) => target[key]({ ...value, projectId });
