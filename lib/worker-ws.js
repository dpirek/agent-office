import { safeTokenEqual, bearerToken } from "./worker-auth.js";
import { acceptWebSocket, canWriteSocket, closeSocket, decodeFrames, sendJson, writeSocket } from "./ws.js";
...
export function createWorkerWebSocketHandler({
  ...
  return function handleWorkerWebSocket(socket, req) {
    ...
    const registrationTimer = setTimeout(() => {
      disconnect("Worker registration timed out.");
      closeSocket(socket, 4000, "Registration required");
    }, 10_000);
    ...
    const heartbeatTimer = setInterval(() => {
      ...
      if (!canWriteSocket(socket)) { disconnect("Worker connection is closing."); return; }
      ...
      try {
        writeSocket(socket, Buffer.from([0x89, 0x00]));
      } catch {
    ...
    const send = (payload) => { if (!disconnected) sendJson(socket, payload); };

    socket.on("data", async (chunk) => {
      if (disconnected || !canWriteSocket(socket)) return;
      ...
      try {
        ...
        for (const frame of decoded.messages) {
          ...
          if (frame.type === "ping") {
            writeSocket(socket, Buffer.concat([Buffer.from([0x8a, frame.payload.length]), frame.payload]));
            continue;
          }
          if (frame.type === "text") {
          ...
          if (!workerName) {
            ...
            const worker = subAgentManager.registerWorker({
              ...
              send,
              close: (code, reason) => {
                disconnect(reason || "Worker connection closed by office.");
          ...
          if (payload.type === 'activity') {
            const record = payload.taskId
              ? subAgentManager.listTasks().find(task => task.taskId === payload.taskId || task.id === payload.taskId)
              : subAgentManager.listDirectMessages().find(message => message.messageId === payload.messageId);
            if (!record || record.agent !== workerName || record.connectionId !== connectionId) continue;
            const event = payload.event;
            if (!event || !['model', 'tool', 'mcp'].includes(event.category)) continue;
            try {
              onActivity({ category: event.category, source: workerName, message: String(event.message || '').slice(0, 500), tone: event.tone,
                metadata: { ...event.metadata, projectId: record.projectId || 'central-office', taskId: record.officeTaskId || record.taskId || null, workerTaskId: record.taskId, messageId: record.messageId } });
            } catch { /* Observability must not disconnect the worker. */ }
