...
export function createWorkerWebSocketHandler({ subAgentManager, getToken = () => "" }) {
  return function handleWorkerWebSocket(socket, req) {
    ...
    socket.on("data", (chunk) => {
      try {
        ...
        for (const frame of decoded.messages) {
          if (frame.type === "close") { closeSocket(socket); return; }
          if (frame.type === "ping") {
          ...
          if (!workerName) {
            ...
            const worker = subAgentManager.registerWorker(payload.worker, {
              connectionId,
              send,
          ...
          if (payload.type === "task_update") {
            const update = subAgentManager.receiveUpdate(workerName, payload, connectionId);
            send({ type: "task_update_ack", ...update });
          } else if (payload.type === "ping") {
            send({ type: "pong", at: Date.now() });
          } else {
            throw new Error("Expected a task_update message.");
          }
        }
      } catch (error) {
    ...
    socket.on("close", () => {
      clearTimeout(registrationTimer);
      subAgentManager.unregisterConnection(connectionId);
    });
  };
}
