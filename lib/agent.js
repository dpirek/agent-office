...
result into your work. When several configured agents can handle independent work, delegate those
assignments in the same turn so they run concurrently. Do not claim a sub-agent was used unless you
called the tool.`;
}

function officeTaskInstructions(tools = []) {
  if (!tools.some((tool) => tool.name === "manage_office_tasks")) return "";
  return `${PROJECT_LAYOUT_POLICY}\n\nUse manage_office_tasks for the persistent office task queue. It can read queued tasks,
create a pending unassigned task, and explicitly assign a pending task to a registered worker.
When creating a task, supply a concise title and a detailed description covering the objective,
relevant context and inputs, constraints, expected deliverables, and acceptance criteria.
The description must let the assigned worker complete the task without the conversation.
When sharing delivered files in chat, use clickable Markdown links with the returned artifact URI
(/files/…) as the destination. Preserve the complete file path; do not use a local filesystem path.
Creating a task must not assign or dispatch it. When asked to process queued work, read the queue
and available workers first, choose a worker deliberately, then make a separate assign tool call.
Do not use delegate_to_sub_agent to dispatch a task that already exists in the office queue.`;
}
...
class CodingAgent {
  ...
  async refinePrompt(prompt, { disabledSteps = [...this.disabledSteps], executionControl } = {}) {
    ...
    const response = await this.client.createResponse({
      ...
      instructions: [
        ...
        new Set(disabledSteps).has("tools") ? "" : officeTaskInstructions(this.tools),
  ...
  async run(prompt, { disabledSteps = [...this.disabledSteps], executionControl } = {}) {
    ...
    const availableTools = this.tools.filter((tool) => {
      if (tool.type === "mcp" || tool.source === "mcp") return !runDisabledSteps.has("mcp");
    ...
    for (let turn = 0; turn < this.maxTurns; turn += 1) {
      ...
      const body = {
        ...
        instructions: [
          ...
          officeTaskInstructions(availableTools),
        ...
        tool_choice: "auto",
        // The model may fan out several independent sub-agent assignments in
        // one turn. Workspace tools are still executed sequentially below.
      ...
      // Delegated workers operate in independent workspaces and use the
      // idempotent message IDs required by the A2A contract, so a model fan-out
      // can safely run concurrently. Local tools remain ordered to avoid races.
      const isDelegationFanOut = calls.length > 1 && calls.every(
        (call) => call.name === "delegate_to_sub_agent",
      );
      if (isDelegationFanOut) {
        input = await Promise.all(calls.map(executeCall));
      } else {
        input = [];
        for (const call of calls) input.push(await executeCall(call));
      }
    }
