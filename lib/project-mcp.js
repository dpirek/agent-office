...
export async function callProjectTool({ uiStateStore, sharedWorkspaceRoot, subAgentManager }, projectId, name, args = {}) {
  ...
  if (name === "project_get_context") return { project, tasks: tasks.map(({ id, title, description, status, agent, dependsOn, deliveredWork }) => ({ id, title, description, status, agent, dependsOn, deliveredWork })) };
