...
const schema = (properties = {}, required = []) => ({ type: "object", properties: { projectId: { type: 'string', description: 'Project this request relates to. Must match the assigned project.' }, ...properties }, required, additionalProperties: false });
...
export function projectMcpTools(projectId) {
  return PROJECT_MCP_TOOLS.map(tool => ({ ...tool, inputSchema: { ...tool.inputSchema,
    properties: { ...tool.inputSchema.properties, projectId: { ...tool.inputSchema.properties.projectId, const: projectId } },
    required: [...tool.inputSchema.required, 'projectId'],
  } }));
}
...
export async function callProjectTool({ uiStateStore, sharedWorkspaceRoot, subAgentManager }, projectId, name, args = {}) {
  ...
  if (args.projectId !== undefined && args.projectId !== projectId) throw new Error('Requested project does not match the authenticated MCP endpoint.');
