...
import { formatConversationContext } from "./conversation-context.js";
import {
...
export function canWriteSocket(socket) {
  return !closingSockets.has(socket) && !socket.destroyed && !socket.writableEnded && !socket.writableFinished && socket.writable !== false;
}

export function writeSocket(socket, data) {
  if (!canWriteSocket(socket)) return false;
  return socket.write(data);
}

export function acceptWebSocket(socket, req) {
...
export function sendFrame(socket, payload) {
  if (!canWriteSocket(socket)) return;
  ...
  writeSocket(socket, Buffer.concat([header, data]));
}
...
export function decodeFrames(buffer) {
  ...
  while (offset + 2 <= buffer.length) {
    ...
    else if (opcode === 0xa) messages.push({ type: "pong", payload });
...
function promptWithHistory(prompt, history) {
  ...
  const messages = history
    ...
    .map((message) => {
      ...
      return { label: role, text: message.text, isUser: message.role === "user" };
    });
  if (!messages.length) return prompt;
  return formatConversationContext(messages, prompt);
}
...
export function createWebSocketHandler({
  ...
  return function handleWebSocket(socket, req) {
    ...
    const heartbeat = setInterval(() => {
      if (awaitingPong || !canWriteSocket(socket)) {
        recordConnection('disconnected', 'Connection heartbeat timed out.');
        socket.destroy();
        return;
      }
      awaitingPong = true;
    ...
    const recordConnection = (state, detail = "") => {
      if (state === "disconnected" && connectionClosed) return;
      if (state === "disconnected") { connectionClosed = true; clearInterval(heartbeat); }
      onConnectionEvent(state, { detail, remoteAddress: socket.remoteAddress || "" });
    };
    ...
    let running = false;
    let currentRun = null;
    const emit = (payload) => { if (!connectionClosed) sendJson(socket, payload); };

    socket.on("data", async (chunk) => {
      if (connectionClosed || !canWriteSocket(socket)) return;
      try {
        ...
        for (const message of decoded.messages) {
          ...
          if (message.type === "close") {
            recordConnection("disconnected", "Client closed the connection.");
            closeSocket(socket);
            return;
          }
          if (message.type === "pong") { awaitingPong = false; continue; }
          if (message.type === "ping") {
            writeSocket(socket, Buffer.concat([Buffer.from([0x8a, message.payload.length]), message.payload]));
            continue;
          }
          let text;
          ...
          if (!authorizeMessage(req, payload)) {
            emit({ type: 'error', error: 'Use project chat for project-scoped requests.' });
            continue;
          }
          ...
          try {
            ...
            const contextualPrompt = promptWithHistory(payload.prompt, payload.history);
            const composedPrompt = disabledSteps.includes("composer")
              ? contextualPrompt
              : await agent.refinePrompt(contextualPrompt, {
                disabledSteps,
        ...
        recordConnection("disconnected", `Protocol error: ${error.message}`);
        closeSocket(socket, 1002, "Protocol error");
      }
    });
    socket.on("end", () => { recordConnection("disconnected", "Network stream ended."); socket.destroy(); });
    socket.on("error", (error) => { recordConnection("disconnected", error.message); socket.destroy(); });
    socket.on("close", () => recordConnection("disconnected", "Socket closed."));
...
export { promptWithHistory };

export function attachWebSocketServer(server, handleWebSocket, pathname = "/ws", additionalHandlers = {}) {
  server.on("upgrade", (req, socket) => {
    const url = new URL(req.url, `http://${req.headers.host || "localhost"}`);
    const handler = url.pathname === pathname ? handleWebSocket : additionalHandlers[url.pathname];
    if (!handler) {
