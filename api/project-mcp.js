...
export function createProjectMcpHandlers(options) {
  return { "/mcp/projects/": async (req, res, url) => {
    ...
    const log = (message, metadata = {}, tone = 'tool') => options.uiStateStore.recordSystemActivity?.({
      category: 'mcp', source: 'Office MCP', message, tone,
      metadata: { projectId, taskId, durationMs: Date.now() - startedAt, ...metadata },
    });
    let request;
    ...
    const error = (code, message) => {
      log(message, { method: request?.method, tool: request?.params?.name, code }, 'error');
      return json(res, 200, { jsonrpc: "2.0", id: request?.id ?? null, error: { code, message } });
    };
    ...
    log(`MCP request · ${request.method}${request.params?.name ? ` · ${request.params.name}` : ''}`, { method: request.method, tool: request.params?.name, arguments: request.params?.arguments });
    let result;
    ...
    log(`MCP ${result.isError ? 'failed' : 'completed'} · ${request.params?.name || request.method}`, { method: request.method, tool: request.params?.name, isError: Boolean(result.isError), error: result.isError ? result.content : undefined }, result.isError ? 'error' : 'success');
