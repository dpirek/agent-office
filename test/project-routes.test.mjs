...
test("project routing handles deep links, legacy links, switching projects and browser history", (t) => {
  ...
  window.location.pathname = '/settings/provider';
  events.get('popstate')();
  assert.equal(tab, 'provider');
...
test('root redirects to registration while dashboard serves the application', async () => {
  for (const url of ['/', '/?campaign=launch', '/dashboard']) {
    ...
    if (url.startsWith('/dashboard')) {
      assert.equal(response.status, 200);
      assert.match(response.body, /<office-shell>/);
    } else {
      assert.equal(response.status, 302);
      assert.equal(response.headers.location, '/register');
