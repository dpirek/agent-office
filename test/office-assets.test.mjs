...
const avatarDirectory = path.join(root, "public", "assets", "avatars");
const spriteNames = [
...
test("agent office has eight RGBA avatar sprites", () => {
  for (const name of spriteNames) {
    const avatar = fs.readFileSync(path.join(avatarDirectory, `${name}.png`));
    assert.equal(avatar.toString("ascii", 1, 4), "PNG");
    assert.equal(avatar[25], 6, `${name} avatar should have an alpha channel`);
  }
});
